...
- **WordPiece tokenizer:** ლექსიკონი ნასწავლია ჩვენივე კორპუსზე (SQuAD-ის
  პარაგრაფები და შეკითხვები). ჩვენმა მოდელმა იცის სპეციფიკური ტერმინების (მაგ. embeddings, skip-gram, lemmatization) დამუშავება და მათ უცნაურ    
...
- **projection ფენა (384→256)**: ტრანსფორმერიდან ინფორმაცია 384-განზომილებიანი გამოდის, ჩვენ კი მას Linear Layer-ით 256-მდე ვკუმშავთ. 
...
მოდელის სწავლება ორი თანმიმდევრული ეტაპისგან შედგებოდა: პრეტრენინგი და ძირითადი, კონტრასტული ტრენინგი.
...
იფარება (masking) და მოდელი აფრედიქთებს მათ. ეს აძლევს მოდელს ენის ბაზისურ
ცოდნას კონტრასტული ეტაპის წინ.
...
pretrained ენკოდერს შემდეგ ვატრენინგებთ **სიმეტრიული InfoNCE** loss-ით (query, მონაკვეთი)
წყვილებზე. ბეჩში ყველა სხვა დოკუმენტი ნეგატივია, ამიტომ 256-იან ბეჩზე თითო
შეკითხვას 255 ნეგატივი ხვდება. რადგან SQuAD-ში ერთ მონაკვეთზე რამდენიმე შეკითხვა
მოდის, ბეჩში დუბლირებული პოზიტივები იფარება, რომ სწორი მონაკვეთი არასოდეს გამოიყენებოდეს
საკუთარ ნეგატივად (`src/losses.py`).

**რატომ InfoNCE?** 256-იან ბეჩზე თითო შეკითხვა იღებს 255 უფასო ნეგატივს — ეს ბევრად
...
**model selection:** ვალიდაცია ითვლის სრულ-კორპუსზე
Recall@k-ს (თითო ვალიდაციის შეკითხვა ფასდება სრულ 18,891 მონაკვეთთან), და არა in-batch
...
შემდეგ მოდელი იწყებს ტრეინინგ წყვილების ოვერფიტს განზოგადების ნაცვლად, რაც
მოსალოდნელია ამ ზომის მოდელისთვის ~70 ათას წყვილზე.
...
ორივე გამოთვლილია სრულ კორპუსზე ყველა სატესტო შეკითხვისთვის.
...
ბი-ენკოდერი კონკურენტუნარიანია sparse baseline-ებთან და Recall@10-ზე BM25-ს მხოლოდ
~1.3-ით ჩამორჩება. baseline-ები წინ არიან Recall@1-ზე, სადაც სიტყვების ზუსტი
დამთხვევა გადამწყვეტია — შეკითხვები, რომლებიც მონაკვეთის იშვიათ ტერმინებს სიტყვა-სიტყვით
იმეორებენ, ლექსიკურ მეთოდებს უწყობენ ხელს. ნეირონული მოდელის უპირატესობა სემანტიკურია:
ის პოულობს მონაკვეთებს, რომლებიც პასუხობენ შეკითხვას, თუნდაც საერთო სიტყვები მცირე იყოს —
რასაც დემო აჩვენებს.

მნიშვნელოვანია კონტექსტი: 18.8-მილიონპარამეტრიანი მოდელი, რომელსაც წინასწარი ცოდნა
არ ჰქონია, სწორ პასუხს top-10-ში ახვედრებს შემთხვევების 71%-ში.

### 6.4 თვისებრივი ანალიზი — ძიება Jurafsky & Martin-ის წიგნში

მოდელი აენკოდირებს შეკითხვას და კოსინუსური მსგავსებით პოულობს უახლოეს მონაკვეთებს
წიგნში (`models/embeddings/`). რამდენიმე მაგალითი:

- **„How does beam search decoding work?"** → 1-ლ ადგილზე beam search-ის სწორი
  სექცია (`jm_0335`), მე-2/მე-3 ადგილზე დანარჩენი beam search სექციები.
...
- **„What is the difference between stemming and lemmatization?"** → 1-ლ ადგილზე
  მორფოლოგიის ზოგადი სექცია (`jm_0366`), ხოლო lemmatization-ის უშუალო განმარტებები
  მე-2/მე-3 ადგილზე (`jm_0060`, `jm_0032`) — ახლოს იყო, მაგრამ ბოლომდე სწორი არა.
...
ეს აჩვენებს, რომ მოდელს შეუძლია ნამდვილი სემანტიკური ძიება - გამოიყენოს SQuAD/ვიკიპედიის
ტრენინგ მონაცემებიდან მიღებული ცოდნა უცნობ ტექსტზე საძიებოდ.
...
- BM25 რჩება უკეთესი Recall@1-ზე ზუსტი ტერმინოლოგიის შემთხვევაში. მაღალი
  სიზუსტის საჭიროების შემთხვევაში სასურველი იქნებოდა ჰიბრიდული სისტემა.
- in-batch ნეგატივები „მარტივი" ნეგატივებია — მოდელს არ უნახავს რეალურად რთული
  (სემანტიკურად მსგავსი, მაგრამ არასწორი) ნეგატივები.
- ჰიპერპარამეტრები (τ, learning rate, batch size) აღებულია ლიტერატურიდან და არ
  გვიცდია ტუნინგი.
